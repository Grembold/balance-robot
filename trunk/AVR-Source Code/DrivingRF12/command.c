...
char text[MAX_BUF];	// Zeichenkette	(String) von Uart
char *ptext = text;	// Hilfspointer auf Zeichenkette
...
void command_put(char last_c)
{
	...
	{				
		...
		{
			// Zeichen ignorieren
			if((last_c == 10) ||(last_c == 8) || (nText == (MAX_BUF-4)) )
			{	
				command = CMD_NOP;
				return;
			}
			...
			*ptext++ = last_c;									// Zeichen Speichern
			rf12_txdata((char*)&last_c, 1);		// ausgeben
			nText++;			
			command = CMD_NOP;
			return;
		}
