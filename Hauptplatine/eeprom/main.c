...
int main(void)
{
    ...
    DDRD  = 0xff;                              // use all pins on port B for output 
    PORTD = 0x00;                              // (active low LED's )
    ...
    if ( ret ) {
        ...
        i2c_stop();
        PORTD=0xff;                            // activate all 8 LED to show error */
    }else {
        ...
        PORTD = ret;                           // output byte on the LED's
        ...
        i2c_write(0x01);                        // write data to address 0
        i2c_write(0x26);                        //    "    "   "    "    1
        i2c_write(0x47);                        //    "    "   "    "    2
        i2c_write(0x58);                        //    "    "   "    "    3
        i2c_stop();                             // set stop conditon = release bus
...
void 24c64_write_data(uint16_t *adress, uint8_t data)
{
	uint8_t status;
	status = i2c_start(DevC02+1);
}
